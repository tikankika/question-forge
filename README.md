...
| Package | Channel | What it gives you |
|---------|---------|-------------------|
| **qf-scaffolding** | npm (planned) | An MCP server that guides the methodology (M1–M5): content analysis, assessment planning, question generation, quality assurance, question formatting. |
| **qf-pipeline** | PyPI (planned) | An MCP server for the technical pipeline: validate question markdown, auto-fix, and export a QTI package. |
| **qti-core** | PyPI (planned) | The standalone QTI generation library that `qf-pipeline` builds on — usable on its own to turn question markdown into QTI 2.2. |

Until the packages are published, install from source (see the quick start below).
