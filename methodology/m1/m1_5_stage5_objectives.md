...
**Purpose:** Finalise scope and synthesise learning objectives
...
This is the final M1 stage - outputs become the foundation for M2 (Assessment Planning).
...
**Opening:**
```
Before synthesising learning objectives, I want to establish clear boundaries—
...
Examples:
- Advanced topics: "Assess if foundational, skip if specialised?"
...
- **Remember:** Recall facts, terms, concepts
  - Verbs: Define, list, state, identify, name, recognise
  
- **Understand:** Explain concepts, describe processes
  - Verbs: Explain, describe, summarise, classify, discuss
...
- Remember: Can they recall/recognise without explanation? → Remember
...
**Default to lower level when uncertain** — the claim about what was demonstrably taught is then more defensible.

**Note on Create:** Bloom's revised taxonomy has a sixth level, Create. It is intentionally excluded from M1's objective formulation because Create-level performance (producing original work) is rarely assessable through the platform-delivered question types this framework targets. If your course requires Create-level objectives, assess them through separate coursework rather than this question set.
...
- Key example: [Example from catalogue that demonstrates this]
...
**What We're Doing:**
Claude presents learning objectives synthesised from all our conversations. You validate that they accurately represent your teaching.
...
✅ **Appropriate Level:**
Is the cognitive level (Remember/Understand/Apply/Analyze/Evaluate) appropriate?
...
Don't worry about memorising taxonomy. Just answer:
- "Is this about recalling facts or explaining concepts?"
- "Do students need to apply it or analyse it?"
...
**Total:** [N] objectives
**Status:** ✅ VALIDATED - Ready for M2 (Assessment Planning)
...
Each item below must be actively verified (teacher and Claude together) before it is ticked — do not present this list as pre-completed:
...
All M1 outputs are saved in the M1 output package directory:
- Validated Learning Objectives (this document)
- Content Analysis (from Stage 0)
- Example Catalogue (from Stage 3)
...
2. **Example Catalogue** ([N] examples)
...
**Total M1 Output:** ~1,200-1,500 lines
...
**Ready for:** M2 (Assessment Planning)
...
- Re-analyse content (that's complete)
- Create actual questions (that's M3)
...
3. **STOP and WAIT** for teacher approval to proceed to M2

### M2 Requirements:
Before proceeding to M2 (Assessment Planning), you MUST have:
- ✅ M2 instructions loaded (Assessment Planning documentation)
- ✅ Teacher has reviewed and validated all learning objectives
- ✅ Teacher has reviewed scope boundaries (IN/CONDITIONAL/OUT)
- ✅ Teacher explicitly says: "Start M2" or "Proceed to assessment design"
- ✅ Complete M1 package saved:
  - Stage 0 analysis
  - Validated learning objectives
  - Example catalogue
...
### DO NOT:
- ❌ Start M2 dialogue without teacher approval
- ❌ Proceed without M2 instructions loaded
...
- ❌ Begin question generation (that's M3, not M2)

### Transition Statement:
"M1 (Content Analysis) is now complete. I have created:

FINAL OUTPUTS:
- [N] validated learning objectives across all priority tiers
- Scope boundaries (IN/CONDITIONAL/OUT)
- Example catalogue ([N] instructional examples)
...
To proceed to M2 (Assessment Planning), I need:
1. Your confirmation that all learning objectives are accurate
2. Your confirmation that scope boundaries are correct
3. Explicit instruction to proceed to M2
4. M2 instructions loaded

Please review the validated learning objectives and let me know when you're ready to start M2 (Assessment Planning)."

---

**M1 Status:** ✅ COMPLETE - AWAITING TEACHER APPROVAL TO PROCEED TO M2
